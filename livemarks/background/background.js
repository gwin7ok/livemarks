...
console.log("[Livemarks] background script loaded");
...
const LivemarkUpdater = {
  async init() {
    console.log("[Livemarks] LivemarkUpdater.init start");
  ...
  async updateLivemark(feed, options) {
    ...
    if (!feedData) {
      // Malformed or unparsable feed — log rich context and skip updating.
      try {
    ...
    for (let i = startIndex; i < children.length; ++i) {
      ...
      if (bookmark.type !== "bookmark") {
        try {
          ...
          // If remove fails (e.g. non-empty folder), try removeTree as a
          // fallback. Log any errors for debugging.
          try {
            await browser.bookmarks.removeTree(bookmark.id);
          } catch (ee) {
            console.error("[Livemarks]", "Failed to remove bookmark/folder", bookmark.id, e, ee);
          }
        }
      } else {
